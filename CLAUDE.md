...
- `src/app/about/page.tsx` — decoy page with `robots: { index: false, follow: false }`. Also blocked in `robots.ts` (`disallow: '/about'`). Keep both in sync if it is renamed.
- `src/app/sitemap.ts`, `src/app/robots.ts`, `src/app/manifest.ts` — Next file-based metadata routes. Sitemap currently lists only `/`. Manifest theme color is the brand orange `#fd7e14`.
